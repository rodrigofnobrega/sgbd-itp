...
int listar_dados() {
    ...
    while (sscanf(buffer + pos, "%d;", &coluna_tipos[aux - 1]) == 1) {
        ...
        aux++;
        coluna_tipos = realloc(coluna_tipos, sizeof(int) * aux);
    }
    ...
    coluna_nomes[aux - 1] = strdup(""); // Alocando espaço inicial para uma string vazia
    while (sscanf(buffer + pos, "%[^;\n];", coluna_nomes[aux - 1]) == 1) {
        pos += strcspn(buffer + pos, ";") + 1;
        aux++;
        coluna_nomes = realloc(coluna_nomes, sizeof(char *) * aux);
        coluna_nomes[aux - 1] = strdup(""); // Alocando espaço inicial para uma string vazia
    }
    ...
    while (fgets(buffer, sizeof(buffer), arquivo) != NULL) {
        ...
        tupla[aux - 1] = strdup(""); // Alocando espaço inicial para uma string vazia
        while (sscanf(buffer + pos, "%[^;\n];", tupla[aux - 1]) == 1) {
            pos += strcspn(buffer + pos, ";") + 1;
            aux++;
            tupla = realloc(tupla, sizeof(char *) * aux);
            tupla[aux - 1] = strdup(""); // Alocando espaço inicial para uma string vazia
        }
